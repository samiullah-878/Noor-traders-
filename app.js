...
import {posSetup,renderPOS,posBack,posStop} from './pos-ledger.js?v=2.7.0';
import {stockSetup,renderStock,stockStop,stockReport,billRates,saleStock,inReport,setInPdf,setTolai} from './pos-stock.js?v=2.7.0';
import {saleSetup,renderSale} from './sale.js?v=2.7.0';
import {ppSetup,renderPP,ppLoadBill} from './purchase.js?v=2.7.0';
import {geoSetup,geoWatch,geoStop,geoEnsure,geoOwnerForm} from './geo-guard.js?v=2.7.0';
import {TYPES,money,cents,norm,balance as rawBalance,daily,yesterday,inDaily} from './model.js';
import {shrinkForAI,readPages,pickModel,pickProModel,matchTally,readPurchaseBill,readLabel} from './ai-tally.js?v=2.7.0';
import {tolaiSetup,openTolai,tolaiClick,tolaiReport,setTolaiPdf} from './tolai.js?v=2.7.0';
import {smartSearch,partyScore,notePartyPick,partyPicks,aliasOf} from './smart-search.js?v=2.7.0';
...
function tolaiPdf(){const r=tolaiReport();if(!r){notice('Kuch nahi mila');return}
 $('printArea').innerHTML=`<h1>NOOR TRADERS</h1><h2>Tolai ka hisaab — ${esc(r.title)}</h2>
  ...
  <table class="iv-tbl"><thead><tr><th>Din</th><th>Items</th><th>Waqt</th><th>Packet</th><th>Minute</th><th>Kaam</th></tr></thead><tbody>
  ${r.rows.map(x=>`<tr class="iv-${x.conf}"><td><b>${esc(x.date)}</b><br><small>${esc(x.wd)}</small></td><td><small>${esc(x.items)}</small></td>
