...
import {posSetup,renderPOS,posBack,posStop} from './pos-ledger.js?v=2.16.0';
import {stockSetup,renderStock,stockStop,stockReport,billRates,saleStock,inReport,setInPdf,setTolai,gintiReport,setGintiPdf} from './pos-stock.js?v=2.16.0';
import {saleSetup,renderSale} from './sale.js?v=2.16.0';
import {ppSetup,renderPP,ppLoadBill} from './purchase.js?v=2.16.0';
import {geoSetup,geoWatch,geoStop,geoEnsure,geoOwnerForm} from './geo-guard.js?v=2.16.0';
import {TYPES,money,cents,norm,balance as rawBalance,daily,yesterday,inDaily} from './model.js';
import {shrinkForAI,readPages,pickModel,pickProModel,matchTally,readPurchaseBill,readLabel} from './ai-tally.js?v=2.16.0';
import {tolaiSetup,openTolai,tolaiClick,tolaiReport,setTolaiPdf} from './tolai.js?v=2.16.0';
import {smartSearch,partyScore,notePartyPick,partyPicks,aliasOf} from './smart-search.js?v=2.16.0';
...
function pcHTML(){const d=pcData;if(!d)return '<p class="muted">Abhi PC se koi report nahi aayi. PC par <b>KHATA-DOCTOR.bat</b> ek dafa chalayein — us ke baad yahan har 30 minute taza haal aayega.</p>';
 const t=x=>x?new Date(x).toLocaleTimeString('en-PK',{hour:'numeric',minute:'2-digit'}):'—',rows=d.services||[],ok=rows.filter(r=>r.ok).length,old=Date.now()-(d.at||0)>45*60000;
 const ic=r=>!r.ok?'🔴':/nayi/.test(r.action||'')?'🔵':/chala di|dobara/.test(r.action||'')?'🟡':'🟢';
 const tag=r=>!r.ok?'Band':/nayi/.test(r.action||'')?'Nayi lagi':/chala di/.test(r.action||'')?'Dobara chali':'Chal rahi';
 return `<div class="tl-sum ${ok===rows.length&&!old?'g':ok>=rows.length-1?'y':'r'}"><div><b>${ok} / ${rows.length}</b><small>chal rahi</small></div><div><b>${t(d.at)}</b><small>aakhri jaanch</small></div><div><b>${esc(d.manifest||'—')}</b><small>version</small></div></div>
  <p class="tl-note">${esc(d.host||'PC')} · on hua ${t(d.bootAt)} · GitHub ${t(d.lastGit)}${(d.changed||[]).length?' · nayi: '+esc(d.changed.join(', ')):''}${old?' · <b class="red">45 min se report nahi — PC band ya doctor ruka?</b>':''}</p>
