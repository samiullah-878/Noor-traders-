...
import {posSetup,renderPOS,posBack,posStop} from './pos-ledger.js?v=2.6.0';
import {stockSetup,renderStock,stockStop,stockReport,billRates,saleStock,inReport,setInPdf} from './pos-stock.js?v=2.6.0';
import {saleSetup,renderSale} from './sale.js?v=2.6.0';
import {ppSetup,renderPP,ppLoadBill} from './purchase.js?v=2.6.0';
import {geoSetup,geoWatch,geoStop,geoEnsure,geoOwnerForm} from './geo-guard.js?v=2.6.0';
import {TYPES,money,cents,norm,balance as rawBalance,daily,yesterday,inDaily} from './model.js';
import {shrinkForAI,readPages,pickModel,pickProModel,matchTally,readPurchaseBill} from './ai-tally.js?v=2.6.0';
import {smartSearch,partyScore,notePartyPick,partyPicks,aliasOf} from './smart-search.js?v=2.6.0';
...
function inMaalPdf(){const r=inReport();if(!r){notice('Kuch nahi aaya');return}
 $('printArea').innerHTML=`<h1>NOOR TRADERS</h1><h2>Aaya hua maal — ${esc(r.branch)}</h2>
  <p>${esc(r.din)} · ${r.rows.length} items · aaya ${esc(String(r.kulQ))} · nikla ${esc(String(r.kulB))}${r.red?` · <b>${r.red} stock mein nahi</b>`:''} · ${esc(new Date().toLocaleString('en-PK'))}</p>
  <table><thead><tr><th>Item</th><th>Kahan se</th><th>Aaya</th><th>${esc(r.branch)}</th><th>Doosra godam</th><th>Nikla</th></tr></thead><tbody>
  ${r.rows.map(x=>`<tr><td>${esc(x.name)}${x.conf==='r'?' (stock mein nahi)':''}</td><td><small>${esc(x.src)}</small></td><td>${esc(x.aaya)}</td><td>${esc(x.here)}</td><td>${esc(x.away)}</td><td>${esc(x.bika)}</td></tr>`).join('')}
  </tbody></table>`;
 openReportPreview('Aaya hua maal '+r.branch);}
